    ...
    <main className="max-w-[80rem] m-auto py-6">
      <div className="bg-stone-100 rounded-2xl font-semibold shadow-lg shadow-black p-8 flex justify-between gap-6 text-slate-900">
        ...
        <div className="flex flex-col justify-between gap-4 w-[33%]">
          <div>
            <Image src={`/pepper_photos/${pepper.image}`} alt={pepper.name} width={200} height={200} className='rounded-2xl w-full object-contain mb-4 shadow-lg shadow-stone-600' />
          ...
          <button className='bg-red-800 text-stone-100 font-bold p-2 rounded-3xl w-[8rem] text-sm uppercase shadow-sm shadow-black text-center'>Purchase</button>
        </div>
        <div className="flex flex-col justify-between gap-6 border shadow-lg shadow-stone-600 p-6 rounded-2xl w-[33%]">
          <div>
            ...
            {pepper.secondaryName != null && (
              <p>Know As: <span className="font-bold">{pepper.secondaryName}</span></p>
            )}
          </div>
          ...
          <div>
            <p className="font-bold text-4xl text-yellow-600">{formattedPrice}</p>
        ...
        <div className="flex flex-col gap-6 border shadow-lg shadow-stone-600 p-6 rounded-2xl w-[33%]">
          <div>
            ...
            <ul className="list-disc ml-6">
              {pepper.uses.map((use, index) => (<li key={index}>{use}</li>))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="list-disc ml-6">
              {pepper.recipes.map((recipe, index) => (<li key={index}>{recipe}</li>))}
            </ul>
          </div>
        </div>
      </div>
